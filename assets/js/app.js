(function () {
  ...
  function bindUiEvents() {
    ...
    filterGroup.addEventListener("click", (event) => {
      ...
      filterGroup.querySelectorAll("button").forEach((item) => item.classList.toggle("is-active", item === button));
      renderChart(false);
    });
  ...
  function resetView() {
    ...
    filterGroup.querySelectorAll("button").forEach((button) => button.classList.toggle("is-active", button.dataset.filter === "all"));
    panelApi.renderNode(panel, root, index, { locked: false });
    renderChart(true);
  }
